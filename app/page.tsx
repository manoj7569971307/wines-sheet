...
// Sample data types are inline below
...
const Home: React.FC = () => {
    ...
    const [selectedShopForLogin, setSelectedShopForLogin] = useState<string>('');
    const [password, setPassword] = useState('');
    const [showPassword, setShowPassword] = useState(false);
    const [loginError, setLoginError] = useState('');

    // In-memory storage for all data
    const [storageData, setStorageData] = useState<Record<string, InventoryMap>>({});
    ...
    const loadInventoryForDate = (date: string) => {
        const key = `inventory_${date}`;
        const inv = storageData[key];
        setInventory(inv && typeof inv === 'object' ? inv : {});
    };

    const saveInventory = () => {
        const key = `inventory_${selectedDate}`;
        setStorageData((prev) => ({
            ...prev,
            [key]: inventory,
        }));
    ...
    const addOrUpdateWine = (wineData: Omit<Wine, 'id'>) => {
